...
exports.handler = async (event) => {
  ...
  if (!address || address.length > 100) return json(400, { error: "Enter a complete U.S. street address (up to 100 characters)." });
  try {
    const headers = { Accept: "application/json", "User-Agent": "Mozilla/5.0 (compatible; LocationLens/1.0)" };
    const geocoderQuery = new URLSearchParams({ address, benchmark: "4", vintage: "4", layers: "8", format: "json" });
    const geocoderResponse = await fetch(`https://geocoding.geo.census.gov/geocoder/geographies/onelineaddress?${geocoderQuery}`, { headers });
    ...
    return json(200, { matchedAddress: match.matchedAddress, name, population: Number(population), income: Number(income), age: Number(age) });
  } catch (error) { return json(502, { error: error.message || "Unable to retrieve Census data. Please try again." }); }
};
